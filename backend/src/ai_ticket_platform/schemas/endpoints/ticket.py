...
class FileInfo(BaseModel):
	...
	rows_skipped: int = Field(
		..., ge=0, description="Number of rows skipped due to validation"
	)
	tickets_extracted: int = Field(
		..., ge=0, description="Number of valid tickets extracted"
	)
...
class TicketBase(BaseModel):
	...
	subject: str = Field(
		..., min_length=1, max_length=500, description="Ticket subject/title"
	)
	body: str = Field(
		..., min_length=1, max_length=10000, description="Ticket description/body"
	)
...
class TicketResponse(TicketBase):
	...
	intent_id: int | None = Field(
		None, description="Linked intent ID (None if not yet clustered)"
	)
...
class ClusteringInfo(BaseModel):
	...
	total_tickets_clustered: int = Field(
		..., ge=0, description="Total tickets processed in clustering"
	)
...
class CSVUploadResponse(BaseModel):
	...
	file_info: FileInfo = Field(
		..., description="File metadata including filename, rows processed, etc"
	)
	tickets_created: int = Field(
		..., ge=0, description="Number of tickets created in DB"
	)
	clustering: ClusteringInfo = Field(
		..., description="Clustering results and cache info"
	)
