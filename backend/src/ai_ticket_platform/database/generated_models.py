...
class Article(Base):
	...
	status: Mapped[str] = mapped_column(
		String(20), nullable=False, server_default=text("'iteration'")
	)
	version: Mapped[int] = mapped_column(
		Integer, nullable=False, server_default=text("1")
	)  # TODO: autoincrement versions on update
...
class Category(
	Base
):  # TODO: Create a utility function that checks no circular dependencies exist
...
class Intent(Base):
	...
	id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
	name: Mapped[str] = mapped_column(String(255), nullable=False)
	category_level_1_id: Mapped[int | None] = mapped_column(
		ForeignKey("categories.id"), nullable=True
	)
	category_level_2_id: Mapped[int | None] = mapped_column(
		ForeignKey("categories.id"), nullable=True
	)
	category_level_3_id: Mapped[int | None] = mapped_column(
		ForeignKey("categories.id"), nullable=True
	)
	area: Mapped[str | None] = mapped_column(String(255))
	...
	variant_a_impressions: Mapped[int] = mapped_column(
		Integer, nullable=False, server_default=text("0")
	)
	variant_b_impressions: Mapped[int] = mapped_column(
		Integer, nullable=False, server_default=text("0")
	)
	variant_a_resolutions: Mapped[int] = mapped_column(
		Integer, nullable=False, server_default=text("0")
	)
	variant_b_resolutions: Mapped[int] = mapped_column(
		Integer, nullable=False, server_default=text("0")
	)
	created_at: Mapped[datetime.datetime] = mapped_column(
		DateTime, server_default=text("CURRENT_TIMESTAMP"), nullable=False
	...
	category_level_1 = relationship(
		"Category", foreign_keys=[category_level_1_id], lazy="joined"
	)
	category_level_2 = relationship(
		"Category", foreign_keys=[category_level_2_id], lazy="joined"
	)
	category_level_3 = relationship(
		"Category", foreign_keys=[category_level_3_id], lazy="joined"
	)

	articles: Mapped[List["Article"]] = relationship(
		"Article", back_populates="intent", cascade="all, delete-orphan"
	)
	source_tickets: Mapped[List["Ticket"]] = relationship(
		"Ticket", back_populates="intent"
	)
...
class Ticket(Base):
	...
	intent: Mapped["Intent | None"] = relationship(
		"Intent", back_populates="source_tickets", passive_deletes=True
	)
