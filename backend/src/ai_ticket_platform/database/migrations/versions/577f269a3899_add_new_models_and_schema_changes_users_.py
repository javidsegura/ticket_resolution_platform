...
def upgrade() -> None:
    ...
    sa.CheckConstraint('(level = 1 AND parent_id IS NULL) OR (level > 1 AND parent_id IS NOT NULL)', name='check_parent_by_level'),
    ...
    sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
    sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=True),
    sa.ForeignKeyConstraint(['category_level_1_id'], ['categories.id'], ),
    sa.ForeignKeyConstraint(['category_level_2_id'], ['categories.id'], ),
    sa.ForeignKeyConstraint(['category_level_3_id'], ['categories.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.Column('blob_path', sa.String(length=1000), nullable=False),
    sa.Column('status', sa.String(length=20), server_default=sa.text("'iteration'"), nullable=False),
    sa.Column('version', sa.Integer(), server_default=sa.text('1'), nullable=False),
    sa.Column('feedback', sa.Text(), nullable=True),
    sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
    sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=True),
    sa.CheckConstraint("status IN ('iteration', 'accepted', 'denied')", name='check_article_status'),
...
def downgrade() -> None:
    ...
    sa.Column('timeRegistered', mysql.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=True),
    sa.Column('isAdmin', mysql.TINYINT(display_width=1), server_default=sa.text("'0'"), autoincrement=False, nullable=True),
    ...
    sa.Column('timeRegistered', mysql.DATETIME(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=True),
    sa.Column('click_count', mysql.INTEGER(), server_default=sa.text("'0'"), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['creator_id'], ['user.user_id'], name=op.f('link_ibfk_1')),
