...
async def approve_article(
	...
	try:
		...
		approval_job = queue.enqueue(approve_article_task, article_id, job_timeout="5m")

		logger.info(
			f"[ARTICLE APPROVE] Enqueued approval job {approval_job.id} for article {article_id}"
		)
