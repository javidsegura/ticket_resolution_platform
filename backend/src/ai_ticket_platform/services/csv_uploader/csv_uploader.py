...
async def cluster_tickets_with_cache(tickets_data: List[dict]) -> Dict:
	...
	try:
		...
		clustering_result = await cluster_and_categorize_tickets(
			tickets_data, llm_client
		)
		logger.info(
			f"Clustering completed: {clustering_result.get('clusters_created', 0)} clusters created"
		)
...
async def save_tickets_to_db(
	db: AsyncSession, tickets_data: List[dict]
) -> List[Ticket]:
	"""
