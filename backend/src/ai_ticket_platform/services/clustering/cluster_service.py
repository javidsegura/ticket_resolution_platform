...
async def cluster_and_categorize_tickets(
	tickets: List[Dict], llm_client: LLMClient
) -> Dict:
	"""
	...
	if not ticket_texts:
		...
		return {"total_tickets": 0, "clusters_created": 0, "clusters": []}
	...
	if clients.cache_manager:
		...
		logger.info(
			f"Cache MISS for clustering hash {clustering_hash[:8]}... - calling LLM"
		)
	else:
	...
	try:
		...
		if not llm_client:
			logger.warning(
				"LLM client not initialized, returning empty clustering result"
			)
			clustering_result = {
			...
			clustering_result = llm_clusterer.cluster_tickets(
				llm_client=llm_client, ticket_texts=ticket_texts
			)
			logger.info(
				f"Successfully clustered tickets into {clustering_result.get('clusters_created', 0)} clusters"
			)

		# Store in cache for 30 days (if initialized)
		if clients.cache_manager:
			await clients.cache_manager.set(
				cache_key, clustering_result, CacheTTL.CLUSTERING_TTL
			)
...
def _extract_ticket_texts(tickets: List[Dict]) -> List[str]:
	...
	logger.debug(
		f"Extracted {len(ticket_texts)} valid ticket subjects from {len(tickets)} tickets"
	)
	return ticket_texts
