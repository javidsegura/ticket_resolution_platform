...
def cluster_tickets(llm_client: LLMClient, ticket_texts: List[str]) -> Dict:
    ...
    """
    logger.debug(f"Building clustering prompt for {len(ticket_texts)} tickets")
    ...
    # call LLM
    logger.info("Calling LLM for ticket clustering")
    ...
    logger.debug(f"LLM returned {len(clusters)} clusters")
    ...
    if assigned_tickets != expected_tickets:
        ...
        logger.error(f"Ticket assignment mismatch! Unassigned: {unassigned}, Invalid/extra: {extra}")
        raise ValueError(
            f"Ticket assignment mismatch! Unassigned: {unassigned}, "
            f"Invalid/extra: {extra}"
        )

    logger.info(f"Successfully validated all {len(ticket_texts)} tickets were assigned to clusters")
    return result
