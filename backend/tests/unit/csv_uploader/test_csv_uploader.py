...
class TestClusterTicketsWithCache:
    ...
    async def test_cluster_tickets_calls_cluster_service(self):
        ...
        # Mock the internal clustering function
        with patch("ai_ticket_platform.services.csv_uploader.csv_uploader.clients") as mock_clients:
            mock_clients.cache_manager = None
            with patch("ai_ticket_platform.services.csv_uploader.csv_uploader.cluster_tickets") as mock_cluster:
                mock_cluster.return_value = expected_result

                result = await cluster_tickets_with_cache(tickets, mock_llm_client)
    ...
    async def test_cluster_tickets_passes_correct_llm_client(self):
        ...
        with patch("ai_ticket_platform.services.csv_uploader.csv_uploader.clients") as mock_clients:
            ...
            try:
                ...
                # Function may fail without full setup, which is OK for this test
                pass
        ...
        with patch("ai_ticket_platform.services.csv_uploader.csv_uploader.clients") as mock_clients:
            ...
            with patch("ai_ticket_platform.services.csv_uploader.csv_uploader.cluster_tickets") as mock_cluster:
                ...
                with pytest.raises((RuntimeError, Exception)):
                    await cluster_tickets_with_cache(tickets, mock_llm_client)
...
class TestSaveTicketsToDb:
    ...
    async def test_save_tickets_calls_create_tickets(self):
        ...
        with patch("ai_ticket_platform.database.CRUD.ticket.create_tickets", new_callable=AsyncMock) as mock_create:
    ...
    async def test_save_tickets_passes_db_session(self):
        ...
        with patch("ai_ticket_platform.database.CRUD.ticket.create_tickets", new_callable=AsyncMock) as mock_create:
            ...
            # Verify create_tickets was called with db parameter
            call_args = mock_create.call_args
            assert call_args is not None
            # Either positional or keyword argument
            if call_args.kwargs:
                assert "db" in call_args.kwargs or "tickets_data" in call_args.kwargs
            else:
