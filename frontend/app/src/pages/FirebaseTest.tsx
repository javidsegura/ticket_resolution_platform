  ...
  const testFirebaseConnection = async () => {
    ...
    try {
      ...
      addResult(
        "Sign Up Test",
        "success",
        `User created: ${user.email || "Email not available"}`,
      );
      ...
      try {
        addResult(
          "Sign In Test",
          "pending",
          `Attempting to sign in: ${testEmail}`,
        );
        const signedInUser = await signIn(testEmail, testPassword);
        addResult(
          "Sign In Test",
          "success",
          `Signed in: ${signedInUser.email || "Email not available"}`,
        );
      } catch (err: any) {
        ...
        <CardContent className="space-y-4">
          <div className="p-4 bg-gray-50 rounded-md">
            ...
            <p className={isMockMode ? "text-yellow-600" : "text-green-600"}>
              {isMockMode
                ? "🔶 MOCK Authentication (Firebase not configured or failed)"
              ...
              {JSON.stringify(
                {
                  ...
                  env: {
                    ...
                    VITE_USE_MOCK_AUTH:
                      import.meta.env.VITE_USE_MOCK_AUTH || "NOT SET",
                  },
                },
                null,
                2,
