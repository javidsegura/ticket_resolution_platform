 ...
 */
export const uploadCompanyDocuments = async (files: File[]): Promise<DocumentUploadResponse> => {
  if (!files.length) {
  ...
  if (!response.ok) {
    ...
    throw new Error(message || `Failed to upload documents: ${response.statusText}`)
  }
